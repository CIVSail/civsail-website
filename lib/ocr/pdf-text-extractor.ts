...
export async function POST(req: Request) {
  try {
    ...
    // 🚫 TEMPORARY HARD GUARD FOR PDFs
    if (mimeType === 'application/pdf') {
      // we tried pdfjs-dist and your runtime can’t load it;
      // so fail gracefully instead of 500
      return NextResponse.json(
        {
          success: false,
          error:
            'PDF OCR is not available on this server. Please export/save this sea service letter as an image (PNG/JPG) and upload again.',
          servicePeriods: { extracted: 0, inserted: 0, duplicates: 0 },
          periods: [],
          needsReview: false,
        },
