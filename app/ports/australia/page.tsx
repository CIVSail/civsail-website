/**
 * @file app/ports/australia/page.tsx
 * @description Australia region port landing page
 *
 * @purpose Show the scope of MSC mariner travel to Australia and invite
...
const ports: PortData[] = [
  {
    ...
    name: 'Australia',
    country: 'Australia',
    subtitle: 'Pacific Partner Nation',
    description:
      'Australia serves as a key ally and strategic partner in the Indo-Pacific region, with major ports supporting joint exercises and maritime operations.',
    colorClass: 'australia-card',
    ports: [
      { id: 'sydney', name: 'Sydney' },
      { id: 'port-townsend', name: 'Port Townsend' },
    ],
  },
];

export default function AustraliaPorts() {
  const router = useRouter();

  const navigateToPort = (countryId: string, portId?: string) => {
    if (portId) {
      router.push(`/ports/australia/${countryId}/${portId}`);
    } else {
      router.push(`/ports/australia/${countryId}`);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-900 via-yellow-700 to-green-900 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12 text-white">
          ...
          <h1 className="text-5xl font-bold mb-4 text-shadow">Australia Ports</h1>
          <p className="text-xl opacity-90 max-w-2xl mx-auto">
            Key maritime gateways in the Indo-Pacific supporting joint operations
            and allied partnerships
          </p>
        </div>

        {/* Ports Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 max-w-4xl mx-auto">
          {ports.map((port, index) => (
            ...
            >
              ...
              <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-green-400 to-yellow-500 transform scale-x-0 transition-transform duration-300 group-hover:scale-x-100" />
