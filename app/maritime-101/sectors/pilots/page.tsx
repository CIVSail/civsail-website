...
import { Navigation, ArrowLeft, Clock, DollarSign, Award } from 'lucide-react';
import { Card } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { SectorFeedbackForm } from '@/components/sectors/SectorFeedbackForm';

export default function PilotsSectorPage() {
  const sector = {
    title: 'Pilots',
    subtitle: 'Harbor & River Pilots',
    description: 'Licensed pilots guide vessels through ports, rivers, and restricted waterways. Highest paid maritime profession, extremely competitive entry, long apprenticeship.',
    stats: { rotation: 'On-call rotation', pay: '$200K-700K+/year', benefits: 'Association provided' },
    content: [
      'State pilot licensing requirements',
      'Federal pilot endorsements',
      'Pilot association structure',
      'Apprenticeship programs (10+ years)',
      'Major pilotage grounds in the U.S.',
      'Day-in-the-life of a harbor pilot',
      'Compensation and profit sharing',
      'How to position for pilot career',
    ],
  };
    ...
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-28">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="text-center max-w-3xl mx-auto">
          <div className="inline-flex p-4 rounded-2xl bg-rose-500/20 border border-rose-500/30 mb-8">
            <Navigation className="w-12 h-12 text-rose-400" />
          </div>
          ...
          <p className="text-xl text-rose-400 mb-6">{sector.subtitle}</p>
          ...
          <SectorFeedbackForm sectorTitle="Pilots" />
