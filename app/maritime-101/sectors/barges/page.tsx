...
import { Waves, ArrowLeft, Clock, DollarSign, Award } from 'lucide-react';
import { Card } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { SectorFeedbackForm } from '@/components/sectors/SectorFeedbackForm';

export default function BargesSectorPage() {
  const sector = {
    title: 'Barges',
    subtitle: 'ATB, ITB & Tank Barges',
    description: 'Articulated Tug-Barge (ATB) and Integrated Tug-Barge (ITB) units moving petroleum, chemicals, and dry cargo coastwise. Growing sector with modern tonnage.',
    stats: { rotation: '14/14 - 28/28', pay: '$60K-180K/year', benefits: 'Union contracts' },
    content: [
      'ATB operations and connections',
      'ITB integrated unit operations',
      'Tank barge cargo operations',
      'Dry cargo barge opportunities',
      'Inland waterway barge work',
      'Major ATB/ITB operators',
      'Tankerman PIC certification',
      'Career path from deckhand to captain',
    ],
  };
    ...
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-28">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="text-center max-w-3xl mx-auto">
          <div className="inline-flex p-4 rounded-2xl bg-cyan-500/20 border border-cyan-500/30 mb-8">
          ...
          <p className="text-xl text-cyan-400 mb-6">{sector.subtitle}</p>
          ...
          <SectorFeedbackForm sectorTitle="Barges" />
